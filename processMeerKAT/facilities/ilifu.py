...
class IlifuFacility(FacilityConfig):
    """IDIA Ilifu cluster — ilifu.ac.za."""

    # Resource limits
    total_nodes_limit = 79
    cpus_per_node_limit = 32
    mem_per_node_gb_limit = 232          # standard partition (MB: 237568)
    mem_per_node_gb_limit_highmem = 480  # HighMem partition (MB: 491520)

    # Defaults
    default_container = '/idia/software/containers/casa-6.5.0-modular.sif'
    default_mpi_wrapper = 'mpirun'
    default_account = 'b03-idia-ag'
    default_partition = 'Main'
    default_modules = ['openmpi/4.0.3']

    # Ilifu-specific: node hostname patterns that indicate a SLURM login/compute node
    _SLURM_HOSTNAMES = ('slurm-login', 'slwrk', 'compute')

    def _on_slurm_node(self):
        hostname = platform.node()
        return any(x in hostname for x in self._SLURM_HOSTNAMES)
